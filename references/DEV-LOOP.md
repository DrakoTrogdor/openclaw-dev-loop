...
```
DISCOVER → STEP 1 (docs↔code) → STEP 2 (user text↔code) → STEP 3 (code review)
...
## Adversarial Evaluation (Optional — Recommended for Complex Code)

For large or complex codebases, add a dedicated **evaluator pass** after Step 3:

> Spawn a separate agent whose only job is to find problems — not fix them, not approve them. Its system prompt is explicitly skeptical. It reads the code and the checklist and asks: *"What did the reviewer miss? What assumptions are wrong? What breaks under load or adversarial input?"*

This separation matters because the same agent that wrote or reviewed the code is biased toward approving it. A dedicated skeptic with no prior attachment to the work finds different things.

Feed the evaluator's findings back into Step 3 Phase B before proceeding to Step 4.
